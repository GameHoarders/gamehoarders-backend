...
function ratingHandler(req, res) {
    try {
        // console.log(`https://api.rawg.io/api/games?key=${process.env.API_KEY}&ordering=-rating&dates=${year}-01-01,${year}-${month < 10 ? `0${month}` : `${month}`}-${date}`);
        let gameURL = `https://api.rawg.io/api/games?key=${process.env.API_KEY}&ordering=-rating&dates=${year}-01-01,${year}-${month < 10 ? `0${month}` : `${month}`}-${date < 10 ? `0${date}` : `${date}`}`;
        // console.log(gameURL);
