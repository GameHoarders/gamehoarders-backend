...
function xboxHandler(req, res) {
    try {
        let gameURL = `https://api.rawg.io/api/games?key=${process.env.API_KEY}&platforms=186,14&ordering=-released&dates=${year-4}-01-01,${year}-${month < 10 ? `0${month}` : `${month}`}-${date < 10 ? `0${date}` : `${date}`}&metacritic=80,100`;
        console.log(gameURL);
